...
# Get the Eastern Time Zone (ET) object
easternTz = pytz.timezone("America/New_York")

# Get the Unix timestamp from the current time in the Eastern Time Zone
timestampEt = datetime.now(easternTz).strftime("%Y-%m-%d|%H:%M:%S")
...
def showSave(fig, modelOutput=False, ticker=None):
    ...
    if modelOutput:
        if ticker is not None:
            fileRoot = f"./Output/ModelOutput/{timestampEt}/{ticker}/"
            fileName = fileRoot + mainTitle.replace(" ", "")
        else:
            fileRoot = f"./Output/ModelOutput/{timestampEt}/"
            fileName = fileRoot + mainTitle.replace(" ", "")
    else:
        fileRoot = f"./Output/AnalysisOutput/{timestampEt}/"
        fileName = fileRoot + mainTitle.replace(" ", "")
...
class analyze2:
    ...
    if analyze:
        ...
        closingDf = yf.download(techList, start=start, end=end, progress=False)[
            "Adj Close"
        ]
...
class model:
    ...
    for ticker in tqdm(techList):
        ...
        # logDir = f"./logs/|{timestampEt}|{ticker}"
        ...
        if newTrain:
            ...
            model.save(f"./Model/{timestampEt}/LSTM|{ticker}.keras")

        else:
            model = keras.models.load_model(
                f"./Model/{timestampEt}/LSTM|{ticker}.keras"
            )
        ...
        print("{} Accuracy:\n{}".format(ticker, test["Error (%)"].describe()))
